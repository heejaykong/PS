...
public class Inflearn02_07 {
	...
	public int solution(int[] isCorrects) {
		int score = 0;
		int accumulatedTempScore = 1;
		
		// 맞춘문제를 만나면 accumulatedTempScore 계속 증가하고 총 점수에 누적해서 반영
		// 틀린문제를 만나면 accumulatedTempScore를 1로 다운그레이드
		for (int isCorrect: isCorrects) {
			if(isCorrect == 1) {
				score += accumulatedTempScore;
				accumulatedTempScore++;
			}
			if(isCorrect == 0) {
				accumulatedTempScore = 1;
			}
		}
		return score;
	}
	...
	public static void main(String[] args){
		...
		int[] isCorrects = new int[count];
		for(int i=0; i<count; i++) isCorrects[i] = sc.nextInt();
		
		System.out.println(T.solution(isCorrects));
	}
}
